...
if isSubjName
    ...
    % Read source space from headmodel metadata if available
    if isfield(InputData.etc.brainstorm, 'headmodel_space')
        sourceSpace = InputData.etc.brainstorm.headmodel_space;
        log.info(sprintf("Source space read from headmodel metadata: %s", sourceSpace));
    else
        sourceSpace = char(config.SourceSpace);
        log.info(sprintf("Source space from config: %s", sourceSpace));
    end
