function EEG = HRB_bst_connectivity_coh(InputData, opt)
% HRB_BST_CONNECTIVITY_COH - Compute coherence (mscohere/icoh/lcohere2019).
...
try
    if strcmp(config.Topology,'NxN'); processName='process_cohere1n'; else; processName='process_cohere1'; end
    if useScouts
        sFilesConn = bst_process('CallProcess',processName,sFilesInput,[], ...
            ...
            'removeevoked',0,'cohmeasure',char(config.CohMetric), ...
            'tfmeasure',char(config.TFMethod),'tfedit',tfEdit, ...
            'timeres',char(config.TimeRes),'avgwinlength',config.AvgWinLength, ...
            'avgwinoverlap',config.AvgWinOverlap,'outputmode',outputMode,'source_abs',-1);
    else
        sFilesConn = bst_process('CallProcess',processName,sFilesInput,[], ...
    ...
    log.error(sprintf("HRB_bst_connectivity_coh failed: %s", ME.message));
    rethrow(ME);
end
...
EEG.etc.brainstorm.connectivity_metric        = 'coh';
