function EEG = HRB_bst_connectivity_phase(InputData, opt)
% HRB_BST_CONNECTIVITY_PHASE - Compute phase-based connectivity (PLV/wPLI/ciPLV).
% PLVMetric: wpli (default, VC-robust), plv, ciplv.
% References: Lachaux 1999; Vinck 2011; Bruna 2018.
...
if isKernelShared
    ...
    sFilesInput = bst_process('CallProcess','process_extract_scout',sFiles,[], ...
        ...
        'addrowcomment',1,'addfilecomment',1,'comment', char(config.SaveName));
...
try
    if strcmp(config.Topology,'NxN'); processName='process_plv1n'; else; processName='process_plv1'; end
    if useScouts
        sFilesConn = bst_process('CallProcess',processName,sFilesInput,[], ...
            ...
            'plvmethod',char(config.PLVMetric),'plvmeasure',2, ...
            'tfmeasure',char(config.TFMethod),'tfedit',tfEdit, ...
            'timeres',char(config.TimeRes),'avgwinlength',config.AvgWinLength, ...
            'avgwinoverlap',config.AvgWinOverlap,'outputmode',outputMode);
    else
        sFilesConn = bst_process('CallProcess',processName,sFilesInput,[], ...
    ...
    log.error(sprintf("HRB_bst_connectivity_phase failed: %s", ME.message));
    rethrow(ME);
end
...
EEG.etc.brainstorm.connectivity_metric        = 'phase';
