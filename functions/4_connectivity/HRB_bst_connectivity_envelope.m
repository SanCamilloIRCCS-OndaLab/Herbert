function EEG = HRB_bst_connectivity_envelope(InputData, opt)
% HRB_BST_CONNECTIVITY_ENVELOPE - Compute envelope correlation (penv/oenv).
% EnvMetric: oenv = orthogonalized (Hipp 2012), removes volume conduction.
...
try
    if strcmp(config.Topology,'NxN'); processName='process_henv1n'; else; processName='process_henv1'; end
    if useScouts
    ...
    log.error(sprintf("HRB_bst_connectivity_envelope failed: %s", ME.message));
    rethrow(ME);
end
...
EEG.etc.brainstorm.connectivity_metric        = 'envelope';
