function EEG = HRB_bst_connectivity_gc_spectral(InputData, opt)
% HRB_BST_CONNECTIVITY_GC_SPECTRAL - Compute spectral Granger causality.
% Reference: Geweke (1982).
...
try
    ...
    if strcmp(config.Topology,'NxN')
        if useScouts
            sFilesConn = bst_process('CallProcess','process_spgranger1n',sFilesInput,[], ...
                ...
                'maxfreqres',config.MaxFreqRes,'maxfreq',config.MaxFreq, ...
            ...
            sFilesConn = bst_process('CallProcess','process_spgranger1n',sFilesInput,[], ...
                ...
                'grangerorder',config.GCOrder,'maxfreqres',config.MaxFreqRes, ...
                'maxfreq',config.MaxFreq,'outputmode',outputMode,'source_abs',-1);
        end
    else
        if useScouts
    ...
    log.error(sprintf("HRB_bst_connectivity_gc_spectral failed: %s", ME.message));
    rethrow(ME);
end
...
EEG.etc.brainstorm.connectivity_metric        = 'gc_spectral';
