function EEG = HRB_bst_connectivity_pte(InputData, opt)
% HRB_BST_CONNECTIVITY_PTE - Compute Phase Transfer Entropy.
% Only NxN topology supported in Brainstorm (process_pte1n).
% Reference: Lobier et al. (2014) Neuroimage.
...
try
    ...
    log.error(sprintf("HRB_bst_connectivity_pte failed: %s", ME.message));
    rethrow(ME);
end
...
EEG.etc.brainstorm.connectivity_metric        = 'pte';
