...
% Authors: Ettore Napoli, University of Bologna, 2026
    ...
    if ~config.UseDefaultAnat

        % Retrieve subject index
        [~, iSubject] = bst_get('Subject', SubjName);

        if strlength(config.FreeSurferDir) > 0
            
            % FreeSurfer complete folder
            fsDir = char(config.FreeSurferDir);
            if ~exist(fsDir, 'dir')
                error("HRB:FreeSurferNotFound", "FreeSurfer folder not found");
            end

            log.info(sprintf("Importing FreeSurfer Anatomy from: %s", fsDir));
            db_set_anatomy(iSubject, fsDir);

        elseif strlength(config.MRIFile) > 0 

            % MRI only (.mgz, .nii, .nii.gz)
            mriPath = char(config.MRIFile);
            if ~exist(mriPath, 'file');
                error("HRB:MRINotFound", "MRI file not found");
            end

            log.info(sprintf("Importing MRI from: %s", mriPath));
            db_set_anatomy(iSubject, mriPath);

        else
            error("HRB:NoAnatomy", ...
                "UseDefaultAnat=false but neither MRIFile nor FreeSurferDir was specified.");
        end

    else
        log.info("Using default anatomy (ICBM152 template).");
    end
    
    %% 9. Prepare input files (save to temp if struct in RAM)
    ...
    %% 10. Brainstorm import — 4-way branch
    %
